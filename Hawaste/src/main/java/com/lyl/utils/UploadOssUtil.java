	...
	public static void uploadFile(MultipartFile upfile, String fileName, OssProperties ossProperties) {
		...
		try {
			...
			MinioClient minioClient = MinioClient.builder().endpoint(ossProperties.getEndpoint(),
																	 ...
																	 ossProperties.getSecure()).credentials(
					ossProperties.getAccessKey(), ossProperties.getSecretKey()).build();

			// 检查存储桶是否已经存在
			boolean isExist = minioClient.bucketExists(
					BucketExistsArgs.builder().bucket(ossProperties.getBucketName()).build());
			if (isExist) {
...
//                            build());
			...
			minioClient.putObject(
					PutObjectArgs.builder().bucket(ossProperties.getBucketName()).object(fileName).stream(
							upfile.getInputStream(), upfile.getSize(), -1).build());
			//minioClient.putObject(ossProperties.getBucketName(),fileName,upfile.getInputStream(),upfile
            // .getContentType());
